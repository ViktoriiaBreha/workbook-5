...
public class Library {
    ...
    public List<Item> searchItems(String query) {
        ...
        for (Item item : items.values()) {
            if (item.getTitle().toLowerCase().contains(query) ||
                    ...
                    item.getGenre().toLowerCase().equals(query) ||
    ...
    public List<Magazine> searchByPublisher(String publisher) {
        List<Magazine> magazinesResults = new ArrayList<>();
        List<Item> allResults = searchByCreator(publisher);
        for (Item item : allResults) {
            if (item instanceof Magazine) {
                magazinesResults.add((Magazine) item);
            }
        }
        return magazinesResults;
    }
